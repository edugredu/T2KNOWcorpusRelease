...
For users who have lawfully obtained source texts for the 389 text-excluded records:
...
Full training over all 821 documents requires lawful reconstruction of text-excluded records. The commands below document the exact invocation used for the paper results; GPU resources (A100-40GB for flat models, RTX PRO 6000 for W2NER) are required.

### Flat baseline training (BiomedBERT example)

```bash
# Prepare data (requires reconstructed full-core data)
