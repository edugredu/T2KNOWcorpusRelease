...
This document defines the released data artefacts and the current record formats used by the `T2KNOW` corpus.
...
The recommended benchmark release is **T2KNOW-Core**, the reviewed-only document-disjoint package:
...
- `data/t2know-core-v1.0/text_included/brat_core/` is the public BRAT standoff export for text-included records only; reconstructed full BRAT is generated locally after reconstruction.
...
The split files `trainData.json`, `evalData.json`, and `testData.json` use line-oriented JSON records despite the `.json` extension.
...
```bash
python3 scripts/validate_corpus.py data/t2know-core-v1.0/document_disjoint_hybrid --format json
python3 scripts/validate_corpus.py data/t2know-core-v1.0/text_included/brat_core --format brat
```
...
- total documents: `1810`
- total entities: `83565`
- split counts:
  - `train=1068`
  - `eval=519`
  - `test=223`
...
- The paper should treat `data/t2know-core-v1.0/document_disjoint_hybrid/` and `data/t2know-core-v1.0/text_included/brat_core/` as T2KNOW-Core, the reviewed resource used for the recommended benchmark and headline corpus statistics.
