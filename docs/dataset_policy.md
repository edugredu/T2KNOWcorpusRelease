...
This document fixes the dataset identity for the `T2KNOW` paper and release workflow.
...
The **reviewed core resource** is **T2KNOW-Core**, the 821-abstract manually reviewed corpus described as the main object of the paper.
...
1. **Reviewed core resource**
   - T2KNOW-Core, the 821-abstract manually reviewed corpus
...
The public repository for the release is <https://github.com/edugredu/T2KNOWcorpusRelease>, with release tag `t2know-core-v1.0.0` for the T2KNOW-Core 1.0.0 corpus package. The GitHub release is <https://github.com/edugredu/T2KNOWcorpusRelease/releases/tag/t2know-core-v1.0.0>. The immutable corpus archive is deposited at <https://doi.org/10.5281/zenodo.20082992>.
...
## Source text provenance and reuse

The release follows a rights-aware hybrid model. The complete T2KNOW-Core annotation layer, label schema, split definitions, validation reports, benchmark scripts, source metadata, checksums, and provenance audit are released for all 821 reviewed source documents. The authors can license project-generated annotations, code, documentation, validation scripts, evaluation scripts, and metadata. Third-party scholarly abstract text has a separate rights status.
...
The public-redacted package is sufficient to verify document counts, sentence counts, entity counts, label counts, split membership, source-text redistribution status, same-span counts, and structural offset counts for T2KNOW-Core. Token counts and token-based sentence-length statistics for text-excluded records are computed from the staged full abstract text or checksum-validated reconstructed abstract text using Python `str.split()` over the fixed reviewed sentence strings. Users who cannot lawfully reconstruct and checksum-match all text-excluded records can inspect annotations and run annotation-only evaluation, but cannot independently verify full-core token counts or reproduce full training over all 821 source abstract texts.

- `data/t2know-core-v1.0/text_included/brat_core/`: `1810` `.txt` files and `1810` `.ann` files
