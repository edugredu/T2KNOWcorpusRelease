...
The official benchmark contains 821 reviewed source documents. The public archive is immediately usable for the text-included subset. For the 389 text-excluded records, the public archive releases annotations, offsets, split assignments, source identifiers, provenance, and checksums, but not third-party source abstract text. Users may reconstruct those reviewed abstract records locally only when they can lawfully access the source publications and can pass checksum validation.

Records that cannot be lawfully accessed or checksum-validated remain usable as annotation/provenance metadata, but they cannot be used for local text-based training or offset validation over all 821 reviewed documents.

Source-licence tables are metadata and licence-evidence audits, not legal advice. Users are responsible for ensuring that reconstruction and use of third-party source abstract text complies with their access rights and publisher terms.
...
- `missing_source_text`: no local reconstructed source abstract text was supplied for that `doc_id`.
...
If a text-excluded record fails access or checksum validation, do not substitute another version and do not train on generated or paraphrased text. Treat that record as annotation/provenance metadata only. Text-based benchmark training over all 821 reviewed documents requires successful local reconstruction and checksum validation of every text-excluded record used for training, validation, or testing.
